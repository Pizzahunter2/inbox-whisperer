...
export default function History() {
  ...
  // On mobile, if a detail panel is selected, show it full-screen
  const showDetailOnMobile = isMobile && (selectedMessage || selectedChat);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <MobileHeader title="History" onOpenSidebar={() => setSidebarOpen(true)} />

      {/* On mobile with detail selected: show detail full-screen */}
      {showDetailOnMobile ? (
        <>
          {selectedMessage && (
            <div className="flex-1 bg-card p-6 flex flex-col">
          ...
          {selectedChat && (
            <div className="flex-1 bg-card flex flex-col h-screen">
              ...
              <div className="px-4 md:px-6 pt-4">
                <TabsList>
                  <TabsTrigger value="emails" className="gap-2">
                    ...
                    <span className="hidden sm:inline">Email Actions</span>
                    <span className="sm:hidden">Emails</span>
                  </TabsTrigger>
                  <TabsTrigger value="chats" className="gap-2">
                    <MessageSquare className="w-4 h-4" />
                    <span className="hidden sm:inline">Inbox Chats</span>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[600px]">
                      <thead className="bg-muted/50 sticky top-0">
                        <tr>
                          <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-4 md:px-6 py-3">Sender</th>
                          <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-4 md:px-6 py-3">Subject</th>
                          <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-4 md:px-6 py-3 hidden sm:table-cell">Category</th>
                          <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-4 md:px-6 py-3">Action</th>
                          <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-4 md:px-6 py-3 hidden sm:table-cell">Date</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr key={msg.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => { setSelectedMessage(msg); setSelectedChat(null); }}>
                              <td className="px-4 md:px-6 py-4">
                                <p className="font-medium text-foreground truncate max-w-[120px] md:max-w-none">{msg.from_name || msg.from_email.split("@")[0]}</p>
                                <p className="text-sm text-muted-foreground truncate max-w-[120px] md:max-w-none hidden md:block">{msg.from_email}</p>
                              </td>
                              <td className="px-4 md:px-6 py-4"><p className="text-foreground truncate max-w-[150px] md:max-w-xs">{msg.subject}</p></td>
                              <td className="px-4 md:px-6 py-4 hidden sm:table-cell"><Badge variant="secondary">{categoryLabels[msg.classification?.category || "other"] || "Other"}</Badge></td>
                              <td className="px-4 md:px-6 py-4">
                  ...
                  <div className="divide-y divide-border">
                    {filteredChats.map((chat) => {
                      ...
                      return (
                        <div
                          key={chat.id}
                          className={`flex items-center gap-3 md:gap-4 px-4 md:px-6 py-4 hover:bg-muted/30 cursor-pointer transition-colors ${
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                            ...
                            <Badge variant="secondary" className="text-xs">{msgCount}</Badge>
                            <Button
                              variant="ghost"
                              size="icon"
